module Homework
    ...
    contains
        ...
        integer(4) :: n,m, L, R, Up, Down,tmp,i,indexx
...
indexx=maxloc(max_sum_array, dim=1)!находим индекс максимального элемента из массива макс.сумм элементов подматриц

call mpi_bcast(x1, 1, MPI_INTEGER4, (indexx-1), MPI_COMM_WORLD, mpiErr)
call mpi_bcast(x2, 1, MPI_INTEGER4, (indexx-1), MPI_COMM_WORLD, mpiErr)
call mpi_bcast(y1, 1, MPI_INTEGER4, (indexx-1), MPI_COMM_WORLD, mpiErr)
call mpi_bcast(y2, 1, MPI_INTEGER4, (indexx-1), MPI_COMM_WORLD, mpiErr)
...
deallocate(current_column)
        ...
        subroutine FindMaxInArray(a, Sum, Up, Down)!одномерный алгоритм поиска максимальной подстроки Кадане
            real(8), intent(in), dimension(:) :: a
